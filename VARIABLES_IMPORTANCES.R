...
df_standardized <- read.csv2("STANDARDIZED_DATA.csv", sep=',', dec=".", check.names=F, fileEncoding = "LATIN1")

# Initialize a function to process different variables
process_variable <- function(variable_name, df) {
  data <- df %>%
    select(MFO, VO2max, F0H, PmaxH, all_of(variable_name)) %>%
  ...
  # Transform the list into a dataframe for processing
